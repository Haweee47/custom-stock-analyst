...
PRIORITY_TIERS = [
    (
        7,  # 존속 자체가 걸린 사안. 놓치면 안 된다.
        [
            "상장폐지", "정리매매", "관리종목지정", "의견거절", "부적정", "자본잠식",
            "감사범위제한", "회생절차", "파산", "부도", "영업정지", "횡령", "배임",
        ],
    ),
    (
        6,  # 공시를 제대로 하지 않은 사안. 존속 문제와는 구분한다.
        ["불성실공시법인지정", "공시번복", "공시변경", "풍문또는보도에대한해명"],
    ),
    (
        5,
        [
            ...
            "교환사채발행", "합병결정", "분할결정", "분할합병", "영업양수", "영업양도",
            "주식교환", "주식이전",
        ],
    ),
    (
...
# 위기 신호처럼 보이지만 실제로는 사무적인 공시들.
# '매매거래정지'는 액면병합·주식분할 같은 기술적 사유로도 나가고,
# '미지정'·'지정유예'는 오히려 문제가 없다는 뜻이다.
CRISIS_EXCEPTIONS = [
    "미지정", "지정유예", "액면병합", "변경상장", "전자등록", "주식분할",
    "액면분할", "해제사유", "기재정정)해제",
]
...
def _score(report_name: str) -> int:
    ...
    for score, keywords in PRIORITY_TIERS:
        if any(keyword in report_name for keyword in keywords):
            if score >= 6 and any(word in report_name for word in CRISIS_EXCEPTIONS):
                continue  # 위기 등급만 취소하고 아래 등급으로 계속 본다
            return score
    return 1
