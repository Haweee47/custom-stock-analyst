...
from src.report.report_view import _growth, _num, _signed
...
body {{ font-family: "Nanum"; font-size: 8.6pt; color: #2b2b2b; line-height: 1.5; }}
...
.points {{ background: #fafbfc; border-left: 2.4pt solid {ACCENT}; padding: 7pt 9pt; margin-bottom: 9pt; }}
.points p {{ margin: 0 0 3.5pt; font-size: 8.4pt; }}
.sec {{ margin-bottom: 9pt; }}
.sec h3 {{ font-size: 9.4pt; color: {ACCENT}; margin: 0 0 3.5pt; padding-left: 5pt;
           border-left: 2.4pt solid {ACCENT}; }}
.sec p {{ margin: 0; font-size: 8.4pt; text-align: justify; }}
.half {{ border: .5pt solid {RULE}; border-top: 1.6pt solid {ACCENT}; padding: 6pt 8pt; }}
...
def build_html(row: pd.Series, result: dict, perf: dict | None, tech: dict | None) -> str:
    report = result["리포트"]
...
<table style="margin-top:9pt"><tr>
  <td width="49%" valign="top"><div class="half"><h4>투자자가 확인해야 할 점</h4>{checks}</div></td>
  <td width="2%"></td>
  <td width="49%" valign="top"><div class="half"><h4>리스크 요인</h4>{risks}</div></td>
</tr></table>
...
def render_pdf(row: pd.Series, result: dict, perf: dict | None, tech: dict | None) -> bytes:
    _register_font()
    buffer = BytesIO()
    status = pisa.CreatePDF(
        build_html(row, result, perf, tech), dest=buffer, encoding="utf-8"
    )
    if status.err:
